...
//             'У неё шикарные синие волосы, которые она обычно собирает в очень высокую причёску. Глаза цвета ореха. ' +
//             'Основное занятие — домохозяйка, большую часть времени проводит в заботе о доме, детях и Гомере. Образ ' +
//             'Мардж копирует стереотип провинциальной американской домохозяйки 50-х годов. Мардж — единственный член ' +
//             'семьи, посещающий церковь добровольно. Старается поддерживать нравственность не только своей семьи, но и ' +
//             'всего города. Отлично готовит, особенно славятся её свиные отбивные и зефир. Любимое блюдо — лапша с маслом.',
...
//     mainInfo.innerText = simpson.info;
...
for (const coursesArrayElement of coursesArray) {
  ...
  let courseTitle =document.createElement('h2');
    courseTitle.classList.add('firstLine');
    courseTitle.innerText = coursesArrayElement.title;

  let durations =document.createElement('div');
    durations.classList.add('secondLine');

    let monthDur =document.createElement('p');
    monthDur.classList.add('monthDur');
    monthDur.innerText = `${coursesArrayElement.monthDuration} month`;
    durations.appendChild(monthDur);

    let hourDur =document.createElement('p');
    hourDur.classList.add('hourDur');
    hourDur.innerText = `${coursesArrayElement.hourDuration} hours`
    durations.appendChild(hourDur);

  let allModules =document.createElement('div');
    allModules.classList.add('thirdLine');

    let module = document.createElement('ul');
    module.classList.add('ul')
    allModules.appendChild(module);

        for (const item of coursesArrayElement.modules) {
        let li = document.createElement('li');
        li.innerText = item;
        li.classList.add('list')
        module.appendChild(li);
    }
  course.append(courseTitle, durations, allModules);
}
